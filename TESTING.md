...
1. 「打开项目…」选择 StudioTest → 左侧导航各类型角标应显示 2 卡牌 / 1 遗物 / 2 能力 / 2 药水
2. 导航点「卡牌」→ 列表点 KitchenSink → 效果列表应正确显示嵌套的 if/repeat 块
3. 导航点「遗物」→「＋ 新建」，用「选择图片…」导入任意 png → 路径自动填入，`assets/relics/` 出现文件
4. 底部「一键部署」→ 日志实时滚动，结束后打印指令清单（「日志 ▾」可折叠）
5. 保存后 `project.stsmod.json` 内容无损（嵌套效果结构保持）
6. 界面为 WinUI 风格且跟随系统深浅色（Windows 设置里切换主题验证）
7. **实时预览**：卡牌编辑页右侧显示整卡预览（卡框按类型与卡池换色、卡图、
   耗能、标题、描述占位符与 BBCode 颜色/特效渲染）；改描述或数值应实时刷新；
   有升级数值的卡出现「显示升级后数值」开关（勾选后数字变绿）；卡池填
   人物专属池（如 `XxxCardPool`）时卡框变为该人物主题色。
   遗物/能力/药水显示游戏悬浮提示样式（图标、金色标题、描述、风味/动态描述）
